import { describe } from "~/api/manifest"
...
export const instructions = (): string =>
  [
    ...
    "",
    "To write an entry, call journal.similar for the payee first and use the accounts these books already use for it, then offer everything you mean to write in a single transaction.propose — not one call per entry. Say confidence 1 only when the accounts came from journal.similar or from the reader; put it lower and say why in a phrase when you are choosing them yourself, and those are the ones set aside for a person to look at.",
    "",
